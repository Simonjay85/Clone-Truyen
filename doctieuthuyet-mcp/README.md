...
The v4.2 discovery contract exposes exactly 64 unique tools. The Node registration and the MCP `tools/list` response are tested against the same catalog; the bridge does not hide a tool because a provider or optional capability is unavailable. A provider-dependent call returns a standard error envelope instead.
...
`create_story_package_draft` always creates draft content. If a package chapter omits `chapter_number`, v4.2 assigns sequential numbers starting at 1; the title does not need a numeric prefix. If a later chapter fails, the bridge attempts to permanently remove only the newly created package drafts and returns `PACKAGE_ROLLED_BACK`; an incomplete rollback is reported explicitly as `PARTIAL_FAILURE` with IDs.
...
### Normal-post content, taxonomy, and internal-link tools
...
`patch_post_content` supports bounded `append`, `prepend`, `replace_exact`, `insert_before`, `insert_after`, `remove_exact`, and exact-ID `replace_section_id` operations. The bridge reads the post back and verifies that status, identity, taxonomy, author, and SEO metadata were not changed. Bulk tools return independent per-item results and are bounded to prevent unbounded writes. Link-related tools parse actual anchor `href` values, normalize internal URLs, reject unsafe schemes, identify broken/self/duplicate links, and do not mutate content except for the explicit related-links upsert.

All normal-post content/SEO/taxonomy writes accept optional `expected_modified_gmt`. A stale value returns `CONTENT_CONFLICT` with the expected and actual timestamps. `update_post` only checks duplicate title/slug identity when that field was explicitly supplied and always excludes the post being updated; content-only and excerpt-only updates do not manufacture duplicate checks. List/search posts and category/tag routes return `pagination` metadata with `page`, `per_page`, `returned`, `total_items`, and `total_pages`.

Examples of the v4.2 tool payloads:
...
```json
{
  "patch_post_content": {
    ...
    "needle": "<p>Existing paragraph.</p>",
    "html": "<p>Small internal-link addition.</p>",
    "expected_modified_gmt": "2026-08-19 01:02:03"
  },
  "bulk_patch_post_content": {
    "operations": [
      {"post_id": 123, "operation": "append", "html": "<p>One bounded change.</p>"},
      {"post_id": 124, "operation": "replace_section_id", "section_id": "dtt-related", "html": "<div id=\"dtt-related\"><p>Updated block.</p></div>"}
    ]
  },
  "upsert_post_related_links": {
    "post_id": 123,
    "section_id": "dtt-related",
    "heading": "Đọc thêm",
    "links": [
      {"url": "https://doctieuthuyet.com/ly-du-la-ai/", "anchor": "Lý Dư là ai?"},
      {"url": "https://doctieuthuyet.com/vien-linh-dien-la-ai/", "anchor": "Viên Linh Điện là ai?"}
    ],
    "placement": "append"
  },
  "bulk_set_rank_math_meta": {
    "operations": [
      {"post_id": 123, "seo_title": "Article title | DocTieuThuyet", "focus_keyword": "article keyword"},
      {"post_id": 124, "meta_description": "A reviewed cluster description."}
    ]
  },
  "bulk_set_post_terms": {
    "operations": [
      {"post_id": 123, "categories": [12], "tags": [31, 32]},
      {"post_id": 124, "tags": []}
    ]
  },
  ...
  "audit_post_internal_links": {
    "post_id": 123,
    "include_incoming_sources": true,
    "max_incoming_sources": 50
  },
  "bulk_audit_post_internal_links": {
    "post_ids": [123, 124, 125],
    "include_incoming_sources": false
  }
}
```

For optimistic locking, call `get_post`, capture its `modified_gmt`, and pass that exact value as `expected_modified_gmt` to the subsequent content, SEO, or taxonomy write. If another editor changes the post first, the write returns `CONTENT_CONFLICT` and the caller should re-read before retrying.
