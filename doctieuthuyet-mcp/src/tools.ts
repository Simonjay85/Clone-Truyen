...
export function registerTools(server: McpServer, client: WordPressBridgeClient = wpBridge): void {
  ...
  server.tool("update_post", "Update only explicitly supplied post fields; never changes status.", {
    post_id: postIdSchema, title: postFields.title.optional(), content: z.string().optional(), excerpt: z.string().optional(), slug: postFields.slug,
  ...
  server.tool("list_categories", "List normal post categories only.", { search: z.string().trim().max(120).optional(), per_page: z.number().int().min(1).max(100).optional() }, async (input) => executeTool("list_categories", "list_categories", (requestId) => call(client, "/list-categories", { ...input, per_page: input.per_page ?? 100 }, "list_categories", requestId, undefined, "list_categories")));
  server.tool("search_categories", "Search normal post categories by name or slug.", { query: z.string().trim().min(1).max(120), per_page: z.number().int().min(1).max(100).optional() }, async ({ query, per_page }) => executeTool("search_categories", "search_categories", (requestId) => call(client, "/search-categories", { search: query, per_page: per_page ?? 100 }, "search_categories", requestId, undefined, "search_categories")));
  server.tool("list_tags", "List normal post tags only.", { search: z.string().trim().max(120).optional(), per_page: z.number().int().min(1).max(100).optional() }, async (input) => executeTool("list_tags", "list_tags", (requestId) => call(client, "/list-tags", { ...input, per_page: input.per_page ?? 100 }, "list_tags", requestId, undefined, "list_tags")));
  server.tool("search_tags", "Search normal post tags by name or slug.", { query: z.string().trim().min(1).max(120), per_page: z.number().int().min(1).max(100).optional() }, async ({ query, per_page }) => executeTool("search_tags", "search_tags", (requestId) => call(client, "/search-tags", { search: query, per_page: per_page ?? 100 }, "search_tags", requestId, undefined, "search_tags")));
