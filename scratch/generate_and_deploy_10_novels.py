...
PROGRESS_FILE = "scratch/progress_10_novels.json"
EXISTING_NOVELS_FILE = "existing_novels.json"
...
def load_progress():
    ...
    return {"completed_novels": [], "novel_chapter_progress": {}}
...
def robust_json_parse(raw_str):
    ...
    try:
        ...
        try:
            fixed = fix_invalid_escapes(cleaned)
        ...
        if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
            ...
            try:
                ...
                try:
                    fixed_candidate = fix_invalid_escapes(json_candidate)
                    return json.loads(fixed_candidate)
                except:
                    raise candidate_err
        raise parse_err

def clean_and_split_v12(html_content):
    # Strip all HTML tags completely to get pure text first (guarantees no tag breaking)
    text = re.sub(r'<[^>]+>', ' ', html_content)
    text = re.sub(r'\s+', ' ', text).strip()
    
    # Sentence splitter pattern
    pattern = r'([.!?]+["”\']?\s+)(?=[A-ZĐÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ"“\'\-\d])'
...
def generate_cover_and_sideload(story_id, prompt):
    print(f"Generating book cover via Pollinations AI for story {story_id}...")
    escaped_prompt = urllib.parse.quote(prompt + ", book cover style, no logo, vibrant lighting, highly detailed, masterwork")
    ...
    try:
        ...
        with open(local_img_path, "wb") as f:
            f.write(r.content)
        print(f"✓ Downloaded cover to {local_img_path}")
    except Exception as e:
    ...
    # Call PHP helper
    print("Triggering update_cover_local.php helper on server...")
    payload = {
        "secret": SECRET_TOKEN,
        "post_id": story_id,
    ...
    # Note: Make sure update_cover_local.php is uploaded. Let's upload it via FTP first
    php_helper_code = """<?php
...
?>"""
    ...
    try:
        ...
        print(f"❌ FTP Helper Upload Error: {e}")
        return False
...
def deploy_novel(story):
    ...
    try:
        res = requests.post(f"{WP_URL}/overwrite_story_generic.php", json=payload, timeout=120)
        res.raise_for_status()
        data = res.json()
        if data.get("success"):
            print(f"🎉 SUCCESS! Story {story['id']} ('{story['title']}') published successfully!")
            print(f"  Deleted chapters: {data['deleted_chapters_count']}")
            print(f"  Inserted chapters: {data['inserted_chapters_count']}")
            return True
        else:
            print(f"❌ Server Error publishing story: {data}")
            return False
    except Exception as e:
        print(f"❌ HTTP request to publish story failed: {e}")
        return False

def sync_existing_novels(story_id, title, intro):
    registry = []
    if os.path.exists(EXISTING_NOVELS_FILE):
        try:
            with open(EXISTING_NOVELS_FILE, "r", encoding="utf-8") as f:
                registry = json.load(f)
        except:
            registry = []
            
    # Remove existing entry if any
    registry = [r for r in registry if r.get("id") != story_id]
    ...
    with open(EXISTING_NOVELS_FILE, "w", encoding="utf-8") as f:
        json.dump(registry, f, ensure_ascii=False, indent=2)
    print(f"✓ Synchronized {EXISTING_NOVELS_FILE}")

def main():
    print("=" * 70)
    print("🚀 10-NOVEL MASTER PIPELINE STARTING...")
    print("=" * 70)
    
    progress = load_progress()
    completed_novels = progress.get("completed_novels", [])
    novel_chapter_progress = progress.get("novel_chapter_progress", {})
    
    for novel in NOVELS:
        story_id = novel["id"]
        title = novel["title"]
        
        if story_id in completed_novels:
            print(f"\n[SKIP] Story {story_id} ('{title}') is already completed.")
            continue
            
        print(f"\n" + "=" * 60)
        print(f"✍️ PROCESSING STORY {story_id}: '{title}'")
        print("=" * 60)
        
        # Load draft from disk if any, otherwise initialize empty chapters list
        draft_file = f"scratch/novel_{story_id}_draft.json"
        chapters = []
        if os.path.exists(draft_file):
            try:
                with open(draft_file, "r", encoding="utf-8") as f:
                    chapters = json.load(f)
                print(f"Loaded {len(chapters)} already written chapters from draft.")
            except:
                chapters = []
                
        # Generate chapters
        for ch_outline in novel["chapters_outline"]:
            ch_num = ch_outline["chap_num"]
            title_hint = ch_outline["title_hint"]
            
            # Check if this chapter is already generated
            if ch_num <= len(chapters):
                print(f"  Chapter {ch_num} already written. Skipping generation.")
                continue
                
            print(f"\nGenerating Chapter {ch_num}: {title_hint}...")
            
            # OpenAI System Prompt
            system_prompt = """Bạn là THE GHOSTWRITER - Nhà văn tiểu thuyết mạng sảng văn (y thuật, thương chiến, vả mặt, hào môn) số 1 Việt Nam. Bạn có phong cách viết cực kỳ chi tiết, sảng khoái, đẩy kịch tính lên cực cao rồi lật kèo ngoạn mục.
QUY TẮC VÀNG ĐẠT 10/10 ĐIỂM MASTERPIECE:
1. SHOW, DON'T TELL: Tuyệt đối không viết sáo rỗng như 'hắn vô cùng kinh hoàng', 'cô vô cùng tức giận'. Thay vào đó, hãy mô tả chi tiết vật lý: 'mồ hôi rịn ra ướt đẫm lưng áo sơ mi lụa', 'làn da chuyển màu xám ngoét', 'ngón tay run rẩy bấu chặt vào cạnh bàn gỗ', 'nhịp tim đập loạn xạ', 'gót giày cao gót gõ xuống nền đá hoa cương sắc lạnh'.
2. CHI TIẾT CHUYÊN MÔN CHÂN THỰC: 
   - Nếu là y thuật: Nêu chi tiết đông y, các huyệt đạo lớn (Hợp Cốc, Nhân Trung, Kỳ Môn, Chương Môn, Dũng Tuyền, Thái Xung), mô tả cơ chế lưu thông khí huyết khoa học, các vị thuốc Nam dược (nhân trần, diệp hạ châu, bồ công anh, cỏ mực, bạch hoa xà) với tỉ lệ và cơ chế phục hồi gan/mô tế bào thực tế.
   - Nếu là thương chiến/pháp lý: Nêu rõ các quy trình tài chính (kiểm toán độc lập, sáp nhập ngược, thâu tóm cổ phần chi phối), các cơ quan thực tế (Bộ Công an C03, A05, SCIC, UBCKNN, Cục Bản quyền tác giả), các ngân hàng thật (Vietcombank, Techcombank, Agribank, MB Bank) và các điều khoản luật doanh nghiệp chuẩn xác.
3. BỐI CẢNH VIỆT NAM THUẦN KHIẾT: Mọi nhân vật, bối cảnh, đường phố đều ở Việt Nam (Đà Nẵng cảng Tiên Sa, Hội An sông Hoài, Bình Dương KCN VSIP, Phú Mỹ Hưng quận 7, Nhà hát Lớn Hà Nội, phố cổ Hà Nội...).
4. TRÁNH VIẾT TẮT: Để việc phân tách câu tự động bằng regex chạy 100% chuẩn xác, TUYỆT ĐỐI không viết tắt. Hãy viết đầy đủ 'thành phố Hồ Chí Minh', 'khu công nghiệp', 'ngân hàng thương mại cổ phần ngoại thương Việt Nam' thay vì viết tắt.
5. ĐỘ DÀI CỰC ĐẠI: Viết chi tiết, kéo dài các màn đối thoại sắc bén đanh thép, suy nghĩ nội tâm kéo dài và phân tích y lý/thương lý sâu sắc. Nội dung chương phải dài từ 1000 đến 1500 từ tiếng Việt (6000 - 9000 ký tự). Không viết tóm tắt!
6. ĐỊNH DẠNG: Chỉ sử dụng các thẻ HTML cơ bản như <p>, <strong>, <em>."""

            user_prompt = f"""Hãy viết CHI TIẾT CHƯƠNG {ch_num} của bộ truyện: '{novel['title']}'.
- Tác giả: {novel['author']}
- Giới thiệu thế giới quan: {novel['intro']}
- Tên chương gợi ý: {title_hint}
- Dàn ý chi tiết Chương {ch_num}: {ch_outline['outline']}

{f"- Các chương trước đã viết: {json.dumps([c['title'] for c in chapters], ensure_ascii=False)}" if chapters else ""}

BẮT BUỘC ĐỘ DÀI PHẢI TỪ 1000 ĐẾN 1500 TỪ TIẾNG VIỆT. Hãy kéo dài các cảnh quay, đặc tả tâm lý nhân vật, những màn đối thoại gay cấn.

Hãy trả về định dạng JSON chính xác tuyệt đối như sau:
{{
  "title": "Chương {ch_num}: [Tên chương giật gân, sảng khoái]",
  "content": "Nội dung chương viết hoàn chỉnh bằng Tiếng Việt 100%, định dạng HTML với các thẻ <p>, <strong>, <em>..."
}}"""

            raw_response = call_openai(system_prompt, user_prompt, max_tokens=4500, temperature=0.7)
            
            try:
                chap_data = robust_json_parse(raw_response)
                
                # Apply strict V12 paragraph serialization (every sentence in own <p>)
                v12_content = clean_and_split_v12(chap_data["content"])
                chap_data["content"] = v12_content
                
                # Check word count
                words = count_words(v12_content)
                print(f"  -> ✓ Success! Generated Chapter {ch_num}: {chap_data['title']}")
                print(f"     Word count: {words} words (~ {len(v12_content)} characters)")
                ...
                print(f"❌ Failed to parse Chapter {ch_num} JSON. Save raw response for debug. Error: {e}")
                # Save raw response to scratch for troubleshooting
                debug_file = f"scratch/novel_{story_id}_chap_{ch_num}_error.txt"
                with open(debug_file, "w", encoding="utf-8") as f:
                    f.write(raw_response)
                print(f"Raw response written to {debug_file}")
                # Halt execution to let developer check
                raise SystemExit(f"Fatal error generating Chapter {ch_num}")
                
            time.sleep(2)
            
        # All 8 chapters generated successfully!
        novel["chapters"] = chapters
        
        # Deploy cover and sideload
        cover_success = generate_cover_and_sideload(story_id, novel["cover_prompt"])
        if not cover_success:
            print(f"⚠️ Cover update failed or skipped, proceeding to metadata deployment.")
            
        # Deploy chapters and metadata
        publish_success = deploy_novel(novel)
        if publish_success:
            # Sync registry
            sync_existing_novels(story_id, novel["title"], novel["intro"])
            
            # Update progress
            completed_novels.append(story_id)
            progress["completed_novels"] = completed_novels
            save_progress(progress)
            
            # Clean up local draft file
            if os.path.exists(draft_file):
                os.remove(draft_file)
            print(f"✓ Completed and finalized Story {story_id}")
        else:
            print(f"❌ Failed to deploy Story {story_id}. Draft is saved. Retrying on next run.")
            raise SystemExit("Fatal: Deployment failed.")
            
    print("\n" + "=" * 70)
    print("🎉 ALL 10 PREMIUM NOVELS GENERATED AND DEPLOYED SUCCESSFULLY!")
    print("=" * 70)
