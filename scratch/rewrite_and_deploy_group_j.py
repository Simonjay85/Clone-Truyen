#!/usr/bin/env python3
"""
Master script: Fetch, rewrite to V13 standard, and deploy Group J stories.
Stories: 2190, 2197, 2207, 2217, 2227
...
STORY_LIST = [
    {
        "id": 2190,
        "title_hint": "Thần Tài Chứng Khoán Phố Wall Hà Ngoại",
        "keywords": "thần tài chứng khoán phố wall hà ngoại, chứng khoán, Hoàng Đức Minh, Phan Thị Bảo Châu, Sở GDCK Phương Bắc, HSX, pump and dump, thâu tóm chứng khoán",
        "focus_keyword": "thần tài chứng khoán"
    },
    {
        "id": 2197,
        "title_hint": "THỢ HỒ NGHÌN TỶ",
        "keywords": "thợ hồ nghìn tỷ, thợ hồ, Trần Hùng, Trọng Khải, Bình Dương, Sóng Thần, tổng thầu",
        "focus_keyword": "thợ hồ nghìn tỷ"
    },
    {
        "id": 2207,
        "title_hint": "Thuyền Trưởng Cảng Đà Thành",
        "keywords": "thuyền trưởng cảng đà thành, thuyền trưởng, Lê Quang Vũ, Ngô Thị Hương, Cảng Đà Thành, Tiên Sa, Meridian Port Corp",
        "focus_keyword": "thuyền trưởng cảng đà thành"
    },
    {
        "id": 2217,
        "title_hint": "Kỹ Sư Smart Grid Và Bóng Tối Điện lực Vạn Thịnh",
        "keywords": "kỹ sư smart grid, bóng tối điện lực, Vạn Thịnh, Phan Đức Thành, Nguyễn Thu Trang, lưới điện quốc gia, tham nhũng điện",
        "focus_keyword": "kỹ sư smart grid"
    },
    {
        "id": 2227,
        "title_hint": "Thiên Tài Lập Trình Đà Lạt Sương Mù: Tôi Viết Mã Hoá Lật Đổ Tập Đoàn Tài Chính Phản Bội",
        "keywords": "thiên tài lập trình đà lạt sương mù, thiên tài lập trình, Đà Lạt, Trần Hưng, Minh Khuê, Thịnh Phát, backdoor rửa tiền",
        "focus_keyword": "thiên tài lập trình đà lạt"
    }
]
...
1. SHOW, DON'T TELL VẬT LÝ: Miêu tả chi tiết hành động vật lý, cử chỉ, biểu cảm gương mặt (nét mặt tái mét, mồ hôi lạnh chảy dài, khớp tay siết chặt đến rớm máu, ánh mắt vằn tia máu, nhịp tim đập loạn xạ nghe rõ mồn một), các đồ vật hay số liệu tài chính rõ ràng (như số tài khoản, số tiền cụ thể bằng VNĐ tại các ngân hàng lớn) thay vì nói chung chung.
  ...
  "content": "[Nội dung chương viết hoàn chỉnh bằng Tiếng Việt 100%, định dạng HTML với các thẻ <p> cho TỪNG CÂU VĂN]"
    ...
    for parse_attempt in range(5):
        try:
            ...
            # Normalize keys if model returns alternate keys or wraps in chapter object
            if "chapter" in chap_data and isinstance(chap_data["chapter"], dict):
                for k, v in chap_data["chapter"].items():
                    if "title" in k.lower() and "title" not in chap_data:
                        chap_data["title"] = v
                    if ("content" in k.lower() or "body" in k.lower()) and "content" not in chap_data:
                        chap_data["content"] = v
            
            if "chapter_title" in chap_data and "title" not in chap_data:
                chap_data["title"] = chap_data["chapter_title"]
            if "chapter_content" in chap_data and "content" not in chap_data:
                chap_data["content"] = chap_data["chapter_content"]
                
            # If title/content still missing, do a broad check
            if "title" not in chap_data:
                for k, v in chap_data.items():
                    if "title" in k.lower() and isinstance(v, (str, list)):
                        chap_data["title"] = v
                        break
            if "content" not in chap_data:
                for k, v in chap_data.items():
                    if ("content" in k.lower() or "body" in k.lower() or "text" in k.lower()) and isinstance(v, (str, list)):
                        chap_data["content"] = v
                        break
            
            # If still missing, raise KeyError to trigger retry
            if "title" not in chap_data or "content" not in chap_data:
                raise KeyError("Missing title or content in generated chapter JSON")
...
Hãy viết lại chương này, giữ nguyên cốt truyện nhưng MỞ RỘNG cực kỳ chi tiết, thêm nhiều đoạn đối thoại đấu trí sắc bén, miêu tả vật lý chi tiết hơn nữa để đạt độ dài TRÊN 1000 từ. Vẫn tuân thủ luật HTML mỗi câu văn nằm trong 1 thẻ `<p>` riêng biệt. Trả về JSON y hệt."""
                ...
                # Re-normalize keys for the expanded data as well!
                if "chapter" in chap_data and isinstance(chap_data["chapter"], dict):
                    for k, v in chap_data["chapter"].items():
                        if "title" in k.lower() and "title" not in chap_data:
                            chap_data["title"] = v
                ...
                if "title" not in chap_data or "content" not in chap_data:
                    raise KeyError("Missing title or content in expanded chapter JSON")
    ...
    raise Exception(f"Fatal: Failed to generate Chapter {idx} for '{story_title}' after 5 attempts.")
...
def main():
    print("=" * 60)
    print("🌟 MASTER REWRITE V13 & DEPLOY ENGINE (GROUP J) 🌟")
    print("=" * 60)
    ...
    print(f"GROUP_J_DONE: {deployed_ids}")
