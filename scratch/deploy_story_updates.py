...
try:
    print("=== STEP 1: Running perfect story rewrite and sync script ===")
    result = subprocess.run(["python3", "scratch/update_story_5056_perfect.py"], capture_output=True, text=True)
    ...
    with open('scratch/update_story_title.php', 'rb') as f:
        ftp.storbinary('STOR update_story_title.php', f)
    print("Uploaded update_story_title.php to server root.")
    ftp.quit()
    
    print("\n=== STEP 3: Executing title update script ===")
    url = "https://doctieuthuyet.com/update_story_title.php"
    try:
    ...
    ftp = ftplib.FTP(FTP_HOST)
    ftp.login(FTP_USER, FTP_PASS)
    ftp.delete('update_story_title.php')
    print("Cleaned up update_story_title.php from server.")
    ftp.quit()
