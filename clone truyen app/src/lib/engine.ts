...
export const STORY_IRON_RULES = _CHECKER_RULES_FULL;
export const DEFAULT_IRON_RULES = _CHECKER_RULES_FULL;

// Giáp hồi sinh cho Client-Side Fetch
async function fetchWithRetry(url: string, options: RequestInit, maxRetries = 4, timeoutMs = 300000) {
  let lastErr;
...
export async function callDeepSeek(params: {
  ...
  const isChapterMap = params.taskType === 'chapter_map';
  ...
  }, isChapterMap ? 1 : 4, isChapterMap ? 210000 : 300000);
...
- BẮT BUỘC phải có ÍT NHẤT 2 chương thuộc loại SỤP ĐỔ (Main bị mất lợi thế hoàn toàn, bằng chứng bị hủy, đồng minh phản bội, kẻ địch thắng thế).
- BẮT BUỘC xen kẽ các chương XÂY DỰNG (chuẩn bị, nội tâm, tìm hiểu bí mật).
CORPORATE / PHÁP LÝ / BẰNG CHỨNG:
- Nếu truyện có công ty, HĐQT, cổ đông, ngân hàng, công an, luật sư hoặc tòa án: mỗi chapter phải có legal_stage hợp lý, không nhảy từ manh mối sang bắt giữ/xử án tức thì.
- Mỗi bằng chứng lớn phải leo thang: manh mối -> đối chiếu -> bằng chứng vùng xám bị challenge -> bằng chứng sạch -> xác nhận độc lập.
- Nếu setting Việt Nam hiện đại, ưu tiên phản công bằng luật sư, nhân sự, truyền thông, tài chính, mua chuộc nhân chứng; bạo lực vật lý chỉ dùng khi có hậu quả pháp lý ngay sau đó.
Trả về JSON có mảng 'chapters' gồm các object: { title, plot_summary, chapter_type, has_setback, legal_stage, evidence_ladder_level, evidence_challenge, threat_method, human_moment }. 
Trong đó 'chapter_type' chỉ được chọn 1 trong 3: "HÀNH ĐỘNG", "XÂY DỰNG", "SỤP ĐỔ". 
'has_setback' (boolean) = true nếu main thua/bất lợi thật sự.
LƯU Ý: Nếu dưới 2 chương có has_setback = true, BẠN PHẢI TỰ ĐỘNG SỬA LẠI DÀN Ý!`;
  ...
  // nhưng vẫn giữ các khóa logic để writer legacy không làm rơi evidence/legal plan.
  return parsed.map((item: any) => ({
     chapter: item.chapter || item.episode || 1,
     title: item.title || '',
     outline: [
       item.plot_summary || item.outline || item.summary || '',
       item.legal_stage ? `Legal stage: ${item.legal_stage}` : '',
       item.evidence_ladder_level ? `Evidence ladder level: ${item.evidence_ladder_level}` : '',
       item.evidence_challenge ? `Evidence challenge: ${item.evidence_challenge}` : '',
       item.threat_method ? `Threat method: ${item.threat_method}` : '',
       item.human_moment ? `Human moment: ${item.human_moment}` : '',
     ].filter(Boolean).join('\n'),
     chapter_type: item.chapter_type || item.type || '',
     has_setback: Boolean(item.has_setback),
     legal_stage: item.legal_stage || 'none',
     evidence_ladder_level: item.evidence_ladder_level || 'none',
     evidence_challenge: item.evidence_challenge || 'none',
     threat_method: item.threat_method || 'none',
     human_moment: item.human_moment || '',
  }));
}
...
⑦ CHỐNG MARY SUE — NHÂN VẬT CHÍNH PHẢI TỰA VẤP ÍT NHẤT 1 LẦN: Nếu outline chỉ định chương thất bại, BẮT BUỘC main phải TÍNH SAI thật sự — kế hoạch lỗ hổng, bị địch đọc trước, mất kiểm soát hoàn toàn. Cấm để main vớt vát gọn lẹ ngay trong cùng chương đó.
...
⑩ LOGIC PHÁP LÝ DOANH NGHIỆP: Nếu outline có legal_stage, phải viết đúng stage đó. Cấm "một cuộc gọi là công an bắt/tòa xử/ngân hàng giao sao kê". Nếu chưa đủ căn cứ, đổi thành kiểm toán, đối chất, tạm đình chỉ, triệu tập làm việc, hoặc niêm phong tài liệu.
⑪ EVIDENCE LADDER: Nếu outline có evidence_ladder_level 1-2, bằng chứng chỉ là manh mối, không được thắng. Level 3 phải bị phản bác. Level 4-5 mới có thể dùng để lật kèo nhưng phải có nguồn, cái giá và xác nhận độc lập.
⑫ VIỆT NAM HIỆN ĐẠI: Phản diện doanh nghiệp ưu tiên đánh bằng luật sư, truyền thông, nhân sự, tiền, hồ sơ giả, mua chuộc nhân chứng. Dao kéo/xe tông/bắt cóc chỉ được dùng khi outline đã cài và chương sau có hậu quả công an/y tế rõ.
...
4. Nếu phần outline có Legal stage / Evidence ladder / Threat method / Human moment, bắt buộc biến chúng thành cảnh thật trong chương, không bỏ qua.
...
export function sanitizeChapterTitle(title: string): string { return title.replace(/^(Title:|Tiêu đề:|Tên chương:|Chương \d+:)\s*/i, '').trim(); }
