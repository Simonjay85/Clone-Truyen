...
export function DeepSeekDramaView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      updateDraftSpace('deepseek_drama', { title, prompt, targetChapters, minChapters, maxChaptersLimit, pitchOptions, gradingStatus, storyStyle });
    }, 500);
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      try {
          ...
          // Truncated JSON Repair Logic: find the last complete object in the array
          const lastCommaBrace = text.lastIndexOf('},');
          if (lastCommaBrace !== -1) {
              let fixedText = text.substring(0, lastCommaBrace + 1);
              ...
              try {
                  ...
                  throw new Error("Lỗi JSON bị cắt nửa chừng của DeepSeek, không thể tự động sửa: " + (parseErr as any).message);
              }
          } else {
              // If there's no '},', maybe there's only 1 incomplete object.
              throw new Error("Không thể đọc định dạng của DeepSeek (bị cắt cụt): " + (parseErr as any).message);
          }
      }
      if (data.usage) {
        addApiLog({
...
NHIỆM VỤ CỦA BẠN: Viết trực tiếp (KHÔNG DÙNG JSON) 1 ĐOẠN ĐỀ XUẤT CÁCH SỬA CỤ THỂ CHO BÁC SĨ (Ví dụ: "Đối với Kịch bản 1: Sửa lại cú lật bàn cho ác độc hơn, cho nam phụ phản bội từ tập 1. Còn Kịch Bản 2: ..."). Lời khuyên phải sắc bén, đi thẳng vào trọng tâm để Bác sĩ có thể copy quăng vào lò phẫu thuật (Auto-Fix) ngay. Đừng viết quá dài, gạch đầu dòng rõ ràng.`,
  ...
  const doGradePitches = async (pitchesArray: any[]) => {
     ...
     setIsGradingParam(-1);
     try {
         const res = await agentConceptScorer('deepseek', deepseekKey, selectedModel || 'deepseek-chat', pitchesArray, resolvedGKey2, resolvedGKey3);
