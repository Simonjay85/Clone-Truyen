  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/openai', {
         ...
         body: JSON.stringify({
            apiKey: openAIKey,
            systemPrompt: `Bạn là Đạo Diễn Thiết Lập. TẠO ĐÚNG 5 KỊCH BẢN MICRO-DRAMA KHÁC NHAU ĐỂ LỰA CHỌN.
...
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY CÓ ĐÚNG 5 KỊCH BẢN. (TRẢ VỀ ÍT HƠN ÍT HƠN 5 KỊCH BẢN LÀ LỖI RẤT NẶNG!). 
{
  "pitches": [
    {
      "super_title": "Tự động sáng tạo 1 Tên Truyện duy nhất. Cực kỳ giật gân, chói tai, mang phong cách 'Vả mặt', 'Bóc phốt' mạnh bạo (Ví dụ: Mẹ Chồng Gài Bẫy Con Dâu, Ai Ngờ Bị Quay Xe Lột Sạch Gia Tài!). TUYỆT ĐỐI KHÔNG DÙNG TÊN CHUNG CHUNG.",
      "summary": "Tóm tắt truyện (Viết theo văn phong Review Phim TikTok/Micro Drama: kịch tính, nhồi nhét drama điên rồ, văn phong thô bạo, vả mặt khốc liệt. Phải khiến độc giả lên máu ngay từ 3 câu đầu tiên!)",
      ...
      "suggestedChapters": "Gợi ý số chương lý tưởng (VD: 5 Chương)"
    }
  ]
}`,
...
NHIỆM VỤ CỦA BẠN: Viết trực tiếp (KHÔNG DÙNG JSON) 1 ĐOẠN ĐỀ XUẤT CÁCH SỬA CỤ THỂ CHO BÁC SĨ (Ví dụ: "Đối với Kịch bản 1: Sửa lại cú lật bàn cho ác độc hơn, cho nam phụ phản bội từ tập 1. Còn Kịch Bản 2: ..."). Lời khuyên phải sắc bén, đi thẳng vào trọng tâm để Bác sĩ có thể copy quăng vào lò phẫu thuật (Auto-Fix) ngay. Đừng viết quá dài, gạch đầu dòng rõ ràng.`,
  ...
  const refineSelectedPitches = async () => {
     ...
     try {
         ...
         setIsGradingParam(-1);
         const res: any = await agentConceptScorer('openai', openAIKey, selectedModel || 'gpt-4o-mini', newPOptions);
                       ...
                       return (
                           <div key={idx} className={`bg-[#17172a] rounded-xl border ${isSelected ? 'border-teal-500 shadow-[0_0_20px_rgba(20,184,166,0.2)]' : 'border-white/5'} p-8 relative flex flex-col transition-all overflow-hidden`}>
                             ...
                             <div className="flex flex-wrap gap-3 mb-6">
                               {(pitch as any).suggestedGenres !== undefined && (
                                 <div className="bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-300 px-3 py-1.5 rounded-full text-[11px] font-bold tracking-wide uppercase flex items-center gap-1.5 w-fit">
                                   🏷️ {typeof (pitch as any).suggestedGenres === 'object' ? JSON.stringify((pitch as any).suggestedGenres) : (pitch as any).suggestedGenres}
                                 </div>
                               )}
