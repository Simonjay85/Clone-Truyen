  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/grok', {
         ...
         body: JSON.stringify({
            apiKey: grokKey,
            systemPrompt: `Bạn là Đạo Diễn Thiết Lập của Grok. Hãy tạo kịch bản gai góc, dark và không che đậy. TẠO ĐÚNG 20 KỊCH BẢN MICRO-DRAMA.
...
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY 20 KỊCH BẢN.
{
  ...
  const refineSelectedPitches = async () => {
     ...
     try {
         ...
         setIsGradingParam(-1);
         const res = await agentConceptScorer('grok', grokKey, selectedModel || 'grok-base', newPOptions);
