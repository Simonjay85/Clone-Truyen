...
export function GrokDramaView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  const { grokKey, geminiKey, geminiKey2, geminiKey3, geminiPaidKey, usePaidAPI, addQueueItems, draftSpaces, updateDraftSpace, addApiLog } = useStore();
  ...
  "pitches": [
    {
      ...
      if (data.usage) {
        addApiLog({
          ...
          cost: ((data.usage.promptTokens || 0) * 5 / 1000000) + ((data.usage.completionTokens || 0) * 15 / 1000000)
        });
      }
