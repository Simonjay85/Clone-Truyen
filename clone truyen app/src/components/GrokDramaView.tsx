...
export function GrokDramaView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  const { grokKey, addQueueItems, draftSpaces, updateDraftSpace} = useStore();
  ...
  const draft = draftSpaces['grok_drama'] || {};
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      updateDraftSpace('grok_drama', { title, prompt, targetChapters, pitchOptions, gradingStatus });
    }, 500);
  ...
  const handleGenerateOutline = async () => {
    if (!grokKey) return alert("⚠️ Bạn chưa nhập Grok xAI API Key trong Settings!");
    ...
    try {
      ...
      const res = await fetch('/api/grok', {
         ...
         body: JSON.stringify({
            apiKey: grokKey,
            systemPrompt: `Bạn là Đạo Diễn Thiết Lập của Grok. Hãy tạo kịch bản gai góc, dark và không che đậy. TẠO ĐÚNG 5 KỊCH BẢN MICRO-DRAMA KHÁC NHAU ĐỂ LỰA CHỌN.
...
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY CÓ ĐÚNG 5 KỊCH BẢN.
{
  ...
  const refineSelectedPitches = async () => {
     if (!grokKey) return alert("Chưa có API Key");
     ...
     try {
         ...
         for (let i = 0; i < selectedPitches.length; i++) {
             ...
             const result = await agentPitchRefiner('grok', grokKey, selectedModel || 'grok-base', newPOptions[idx], refineFeedback);
         ...
         setIsGradingParam(-1);
         const res: any = await agentConceptScorer('grok', grokKey, selectedModel || 'grok-base', newPOptions);
  ...
  const gradeAllPitches = async () => {
     ...
     setIsGradingParam(-1);
     try {
         const res = await agentConceptScorer('grok', grokKey, 'grok-base', pitchOptions);
  ...
  const gradePitch = async (index: number) => {
     ...
     try {
        if (!grokKey) return alert("Chưa có Grok xAI Key");
        const res = await fetch('/api/grok', {
           method: 'POST',
           headers: { 'Content-Type': 'application/json' },
           body: JSON.stringify({
              apiKey: grokKey,
              systemPrompt: `Bạn là Đạo Diễn Thiết Lập của Grok. Hãy tạo kịch bản gai góc, dark và không che đậy. TẠO ĐÚNG ${targetChapters} KỊCH BẢN MICRO-DRAMA.
TUYỆT ĐỐI KHÔNG DÙNG TỪ TIẾNG ANH. Viết bằng tiếng Việt hoặc Hán Việt thuần túy. Xây dựng cốt truyện mang màu sắc Châu Á (đặc biệt là Việt Nam hoặc Trung Quốc).
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY CÓ ĐÚNG ${targetChapters} KỊCH BẢN.
{
  ...
  const handlePushAutopilot = () => {
    ...
    const items = selectedPitches.map(idx => ({
        ...
        model: selectedModel,
        outlineEngine: 'grok' as const, writeEngine: 'grok' as const
    }));
    addQueueItems(items);
    setSelectedPitches([]);
    ...
    <div className="w-full h-full flex flex-col font-sans bg-[#0f0f17] text-[#e2e8f0] overflow-hidden animation-fade-in">
      ...
      <nav className="h-auto md:h-[60px] py-4 md:py-0 bg-[rgba(15,15,23,0.9)] border-b border-[rgba(255,255,255,0.07)] px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 shrink-0 z-50 backdrop-blur-md">
        <div className="flex items-center gap-2 font-extrabold text-[18px] text-white tracking-wide">
          🎬 Trạm Nổi Loạn <span className="bg-gradient-to-br from-orange-500 to-red-400 text-white px-2.5 py-0.5 rounded-full text-xs uppercase tracking-wider ml-1">Grok xAI</span>
        </div>
        
        <div className="flex gap-3">
           <button className="px-6 py-2 rounded-lg bg-gradient-to-br from-orange-500 to-red-500 text-white font-bold text-sm shadow-lg">🎬 Đạo Diễn</button>
        </div>
        ...
        <aside className="w-full md:w-[540px] bg-[#13131f] border-b md:border-b-0 md:border-r border-white/5 flex flex-col md:overflow-y-auto custom-scrollbar p-6 shrink-0 z-10 transition-all">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-5">
             <div className="flex justify-between items-center mb-3 text-sm font-semibold text-[#94a3b8]">
                ...
                <button onClick={() => onNavigate && onNavigate('settings')} className="bg-orange-500/15 border border-orange-500/50 px-2 py-1 rounded-lg text-orange-300 flex items-center gap-1.5 text-xs hover:bg-orange-500/30 transition-all">
                   <Settings size={14}/> Settings
                </button>
             </div>
             
             <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-3 text-sm text-orange-300 leading-relaxed font-medium">
               <strong>❖ Grok xAI System:</strong> Model mặc định được thiết lập là <code>grok-beta</code> để sinh kịch bản cực Dark. <br/><br/>Hệ thống Auto-Pilot đằng sau sẽ tự động gọi <code>grok-beta</code> (hoặc các phiên bản rebel) để viết kịch bản và rải độc.
             </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-5">
             ...
             <div className="flex gap-2">
                <div className="flex flex-1">
                   <input type="number" className="w-[54px] bg-red-500/5 border border-red-500/40 border-r-0 rounded-l-lg text-red-500 font-bold text-sm text-center outline-none" title="Số chương mục tiêu" value={targetChapters} onChange={e => setTargetChapters(parseInt(e.target.value) || 1)} />
                   <button onClick={handleGenerateOutline} disabled={isGenerating} className="flex-1 bg-red-500/10 border border-red-500/40 rounded-r-lg text-red-500 font-bold text-xs py-2 hover:bg-red-500/20 transition-all flex items-center justify-center gap-1.5">{isGenerating ? '⏳ Đang xoay...' : '💡 Gợi ý Kịch Bản'}</button>
                </div>
                ...
                {Object.entries(GENRES_GROUPS).map(([groupName, genres]) => (
                   <div key={groupName}>
                     ...
                     <div className="flex flex-wrap gap-2">
                        {genres.map(g => (
                           <button key={g} onClick={() => toggleGenre(g)} className={`px-3 py-1.5 rounded-full border text-xs font-semibold transition-all ${selectedGenres.includes(g) ? 'border-rose-500 text-rose-300 bg-rose-500/20 shadow-[0_0_10px_rgba(244,63,94,0.15)]' : 'border-white/10 text-slate-400 hover:border-rose-500/50 hover:text-rose-300'}`}>{g}</button>
                        ))}
                     </div>
                   </div>
                ))}
        ...
        <main className={`flex-1 bg-[#0a0a10] flex flex-col min-w-0 ${!isConfigExpanded ? 'md:ml-8' : ''}`}>
           ...
           <div className="min-h-[48px] py-3 md:py-0 bg-[#17172a] border-b border-white/5 px-6 flex flex-col md:flex-row justify-between items-center shrink-0 gap-3">
              <div className="flex items-center gap-2 text-sm text-[#6b7a99] font-medium">
                 <div className={`w-2.5 h-2.5 rounded-full ${isGenerating ? 'bg-amber-500 animate-pulse' : pitchOptions.length > 0 ? 'bg-red-500' : 'bg-slate-600'}`}></div>
           ...
           <div className="flex-1 md:overflow-y-auto custom-scrollbar p-10 relative text-[#e2e8f0]">
              ...
              {!isGenerating && pitchOptions.length === 0 && (
                 <div className="h-full flex flex-col items-center justify-center text-center opacity-70">
                    ...
                    <h2 className="text-[22px] font-bold text-[#4b5563] mb-4">Lò Sát Sinh AI (Grok Rebel)</h2>
              ...
              {isGenerating && (
                 <div className="h-full flex flex-col items-center justify-center">
                    ...
                    <div className="text-rose-400 font-bold text-[15px] tracking-[2px] animate-pulse uppercase">AI Đang Lên {targetChapters} Kịch Bản Tuyệt Đỉnh...</div>
                 </div>
              )}
              ...
              {pitchOptions.length > 0 && !isGenerating && (
                 ...
                 <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 animation-fade-in pb-10">
                    {pitchOptions.map((pitch, idx) => {
                       ...
                       return (
                           <div key={idx} className={`bg-[#17172a] rounded-xl border ${isSelected ? 'border-orange-500 shadow-[0_0_20px_rgba(20,184,166,0.2)]' : 'border-white/5'} p-8 relative flex flex-col transition-all overflow-hidden`}>
                             ...
                             <div className="flex justify-between items-start mb-6">
                                <div className="flex items-center gap-4">
                                   <button onClick={() => togglePitchSelect(idx)} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-orange-500 border-orange-500 text-white' : 'border-slate-600 text-transparent hover:border-slate-500'}`}>
                                ...
                                {grading ? (
                                   <div className="bg-black/40 border border-white/10 px-4 py-2 rounded-lg flex items-center gap-3">
                                      ...
                                      <div className={`text-2xl font-black ${grading.score >= 8 ? 'text-red-400' : grading.score >= 6 ? 'text-amber-400' : 'text-red-400'}`}>{grading.score}</div>
                                   </div>
                                ) : (
                             ...
                             {grading && grading.grading && (
                                <div className={`p-4 rounded-xl text-sm font-medium border border-l-4 ${grading.score >= 8 ? 'bg-red-500/10 border-red-500/30 border-l-red-500 text-red-200' : grading.score >= 6 ? 'bg-amber-500/10 border-amber-500/30 border-l-amber-500 text-amber-200' : 'bg-red-500/10 border-red-500/30 border-l-red-500 text-red-200'}`}>
