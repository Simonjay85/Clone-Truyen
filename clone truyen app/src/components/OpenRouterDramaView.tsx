...
export function OpenRouterDramaView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  useEffect(() => {
    ...
    const draft = draftSpaces['openrouter_pipeline'] || {};
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      updateDraftSpace('openrouter_pipeline', { prompt, storyBible, chapterMap, currentState, targetChapters, selectedGenres, autoChapters, riskLevel, chapters, auditReports, customIronRules, useFullIronRulesInChecker, finalAuditReport });
    }, 500);
  ...
  const handleSaveBible = () => {
    updateDraftSpace('openrouter_pipeline', { storyBible });
    showToast('✅ Story Bible đã lưu!', 'success');
  };

  const handleSaveMap = () => {
    updateDraftSpace('openrouter_pipeline', { chapterMap });
  ...
  const handleSaveChapter = () => {
    ...
    updateDraftSpace('openrouter_pipeline', { chapters, currentState: newStateToSave });
    showToast(`✅ Chương ${selectedChapterIdx + 1} đã lưu!`, 'success');
  };

  const handleSaveAudit = () => {
    updateDraftSpace('openrouter_pipeline', { auditReports });
  ...
  const handleResetCurrentStory = () => {
    ...
    updateDraftSpace('openrouter_pipeline', { storyBible: '', chapterMap: [], chapters: [], auditReports: {}, currentState: '{}', finalAuditReport: null });
    showToast('🗑️ Reset thành công! Đã xoá trắng dữ liệu cốt truyện hiện tại.', 'success');
  };

  const handleGenerateBible = async () => {
    if (!openRouterKey) return showToast("Vui lòng nhập OpenRouter API Key trong cài đặt!", 'error');
      ...
      for (let i = apStartChapter; i <= apEndChapter; i++) {
        ...
        const sanitizeChapterOutput = (text: string): string => {
          return stripMetaBlocks(text)
            // Strip OpenRouter R1 <think>...</think> reasoning blocks
            .replace(/<think>[\s\S]*?<\/think>/gi, '')
        ...
        if (apMode === 'draft_only') {
          ...
          updateDraftSpace('openrouter_pipeline', { chapters: localChapters, currentState: localCurrentState });
          ...
          if (apMode === 'balanced') {
            localCurrentState = mergeStateStr(localCurrentState, stateToMerge);
            setCurrentState(localCurrentState);
            updateDraftSpace('openrouter_pipeline', { chapters: localChapters, currentState: localCurrentState, auditReports: localAuditReports });
  ...
  const handleFinalAudit = async () => {
    ...
    try {
      ...
      // Auto save draft
      updateDraftSpace('openrouter_pipeline', { finalAuditReport: result.text });
    } catch (e: any) {
  ...
  const handleAutoFixFromAudit = async () => {
    ...
    if (!openRouterKey) return showToast('Thiếu OpenRouter API Key!', 'error');
    ...
    for (const chapNum of sorted) {
      ...
      try {
        ...
        setChapters([...newChapters]);
        updateDraftSpace('openrouter_pipeline', { chapters: newChapters });
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
             ...
             <div className="bg-[#1a1a1a] rounded-xl border border-white/10 p-6 flex flex-col gap-6">
                <div>
                   ...
                   <p className="text-white/60 mb-4">Nhập ý tưởng thô và OpenRouter R1 sẽ tư duy (Reasoning) để lập ra một Story Bible sắc bén, logic chặt chẽ.</p>
            ...
            <div className="bg-[#1a1a1a] rounded-xl border border-white/10 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* Save Iron Rules */}
                  <button
                    onClick={() => { updateDraftSpace('openrouter_pipeline', { customIronRules }); showToast('✅ Iron Rules đã lưu!', 'success'); }}
