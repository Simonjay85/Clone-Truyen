...
import { STORY_GENRES, StoryGenreId, STORY_GENRE_LIST } from '../config/storyStyles';



export function QwenDramaView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  const { qwenKey, geminiKey, geminiKey2, geminiKey3, geminiPaidKey, usePaidAPI, addQueueItems, draftSpaces, updateDraftSpace, addApiLog } = useStore();
  ...
  const [storyStyle, setStoryStyle] = useState<StoryGenreId>('ngon_tinh');
  ...
  const draft = draftSpaces['qwen_drama'] || {};
  ...
  React.useEffect(() => {
    ...
    setTimeout(() => { 
      if (draft.storyStyle !== undefined && draft.storyStyle !== storyStyle) {
        if (STORY_GENRES[draft.storyStyle as StoryGenreId]) {
          setStoryStyle(draft.storyStyle as StoryGenreId);
        } else {
          setStoryStyle('ngon_tinh');
        }
      } 
    }, 0);
  }, [draft]);
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      updateDraftSpace('qwen_drama', { title, prompt, targetChapters, minChapters, maxChaptersLimit, pitchOptions, gradingStatus, storyStyle });
    }, 500);
    return () => clearTimeout(timer);
  }, [title, prompt, targetChapters, minChapters, maxChaptersLimit, pitchOptions, gradingStatus, storyStyle]);
  ...
  const handleGenerateOutline = async () => {
    if (!qwenKey) return alert("⚠️ Bạn chưa nhập OpenAI API Key trong Settings!");
    ...
    try {
      ...
      const res = await fetch('/api/qwen', {
         ...
         body: JSON.stringify({
            apiKey: qwenKey,
            systemPrompt: (STORY_GENRES[storyStyle] || STORY_GENRES['ngon_tinh']).getPitchPrompt(targetChapters, minChapters, maxChaptersLimit),
            userPrompt: prompt 
              ? `Thể loại: ${genresStr}\nYêu cầu gốc: ${prompt}\n\nLỆNH ÉP BUỘC ĐA DẠNG (EXTREME DIVERSITY): Mặc dù chung 1 yêu cầu gốc, nhưng \${targetChapters} kịch bản PHẢI VÔ CÙNG KHÁC BIỆT. Cấm lặp lại nghề nghiệp, cấm lặp lại bối cảnh, cách đặt tên cũng phải khác concept nhau hoàn toàn. Không được có 2 kịch bản na ná nhau!` 
              : `Thể loại: ${genresStr}\nYêu cầu: Dựa vào thể loại trên, hãy tạo \${targetChapters} kịch bản VÔ CÙNG KHÁC BIỆT NHAU về bối cảnh, nhân vật, twist và cách đặt tên. NGHIÊM CẤM tạo các kịch bản có motip na ná nhau. Mỗi kịch bản là một luồng gió hoàn toàn mới.`,
            jsonMode: true,
            temperature: 1.0,
            model: selectedModel || 'qwen3-max-2026-01-23'
         })
      });
          ...
          if (lastBrace !== -1) {
              ...
              try {
                  ...
                  if (match) {
                      ...
                      throw new Error("Lỗi JSON bị cắt nửa chừng của Qwen: " + (parseErr as any).message);
                  }
              }
          } else {
              throw new Error("Không thể đoc định dạng của Qwen: " + text.substring(0, 100));
          }
      }
      if (data.usage) {
        addApiLog({
      ...
      const slicedPitches = pitches.slice(0, targetChapters);
      setPitchOptions(slicedPitches); 
      if (!title && (slicedPitches[0]?.super_title || slicedPitches[0]?.protagonist)) setTitle(slicedPitches[0]?.super_title || slicedPitches[0]?.protagonist);
      
      // Auto-minimize config panel so user can read pitches
      if (slicedPitches.length > 0) {
        setIsConfigExpanded(false);
        // Automatically grade the newly generated pitches
        doGradePitches(slicedPitches);
      }
    } catch (e: any) {
  ...
  const handleSuggestFix = async () => {
    if (!qwenKey) return alert("Chưa có API Key");
...
NHIỆM VỤ CỦA BẠN: Viết trực tiếp (KHÔNG DÙNG JSON) 1 ĐOẠN ĐỀ XUẤT CÁCH SỬA CỤ THỂ CHO BÁC SĨ (Ví dụ: "Đối với Kịch bản 1: Sửa lại cú lật bàn cho ác độc hơn, cho nam phụ phản bội từ tập 1. Còn Kịch Bản 2: ..."). Lời khuyên phải sắc bén, đi thẳng vào trọng tâm để Bác sĩ có thể copy quăng vào lò phẫu thuật (Auto-Fix) ngay. Đừng viết quá dài, gạch đầu dòng rõ ràng.`,
  ...
  const refineSelectedPitches = async () => {
     ...
     try {
         ...
         for (let i = 0; i < selectedPitches.length; i++) {
             ...
             const result = await agentPitchRefiner('qwen', qwenKey, selectedModel || 'qwen3-max-2026-01-23', newPOptions[idx], refineFeedback, resolvedGKey2, resolvedGKey3);
         ...
         setIsGradingParam(-1);
         const res: any = await agentConceptScorer('qwen', qwenKey, selectedModel || 'qwen3-max-2026-01-23', newPOptions, resolvedGKey2, resolvedGKey3);
  ...
  const doGradePitches = async (pitchesArray: any[]) => {
     if (!qwenKey) return;
     if (pitchesArray.length === 0) return;
     setIsGradingParam(-1);
     try {
         const res = await agentConceptScorer('qwen', qwenKey, selectedModel || 'qwen3-max-2026-01-23', pitchesArray, resolvedGKey2, resolvedGKey3);
         ...
         console.log("Auto-grading failed: ", e.message);
     } finally {
         setIsGradingParam(null);
     }
  };
  ...
  const gradeAllPitches = async () => {
     ...
     try {
        await doGradePitches(pitchOptions);
     } catch (e: any) {
  ...
  const gradePitch = async (index: number) => {
     ...
     try {
        if (!qwenKey) return alert("Chưa có OpenAI Key");
        const res = await fetch('/api/qwen', {
           method: 'POST',
           headers: { 'Content-Type': 'application/json' },
           body: JSON.stringify({
...
Chỉ trả về JSON: {"score": number, "grading": "Nhận xét chi tiết..."}`,
  ...
  const handlePushAutopilot = () => {
    ...
    const items = selectedPitches.map(idx => ({
        ...
        targetChapters: parseInt(pitchOptions[idx].target_chapters) || parseInt(pitchOptions[idx].suggestedChapters) || targetChapters,
        maxChapters: maxChapters,
        model: selectedModel,
        storyStyle: storyStyle,
        outlineEngine: 'qwen' as const, writeEngine: 'qwen' as const
    }));
    addQueueItems(items);
    setSelectedPitches([]);
        ...
        <aside className="w-full md:w-[540px] bg-[#13131f] border-b md:border-b-0 md:border-r border-white/5 flex flex-col md:overflow-y-auto custom-scrollbar p-6 shrink-0 z-10 transition-all">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-5">
             ...
             <div className="bg-teal-500/10 border border-teal-500/30 rounded-lg p-3 text-sm text-teal-300 leading-relaxed font-medium">
                <strong>❖ Qwen System (Alibaba):</strong> Model <code>qwen3-max</code> được thiết lập trọn gói cho Sáng Tác 7. Rất mạnh về văn hóa mạng và cẩu huyết Á Đông.<br/>
                <br/>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-5">
             <label className="block text-sm font-semibold text-[#94a3b8] mb-3">Văn Phong / Thể Loại Truyện</label>
             <div className="grid grid-cols-2 gap-3">
               {STORY_GENRE_LIST.map((genre) => (
                 <button 
                   key={genre.id}
                   onClick={() => { setStoryStyle(genre.id); setSelectedGenres([]); }} 
                   className={`px-2 py-2.5 rounded-lg text-[13px] font-bold border transition-all truncate ${storyStyle === genre.id ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-300 shadow-[0_0_15px_rgba(99,102,241,0.15)]' : 'bg-black/30 border-white/10 text-slate-400 hover:bg-white/5 hover:border-white/20'}`}
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5">
             ...
             <div className="flex flex-col gap-4">
                {Object.entries((STORY_GENRES[storyStyle] || STORY_GENRES['ngon_tinh']).suggestedTags || {}).map(([groupName, genres]) => (
                   <div key={groupName}>
