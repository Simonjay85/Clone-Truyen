  ...
  "pitches": [
    {
      ...
      "suggestedChapters": "Số nguyên (8-25) tùy độ phức tạp cốt truyện như đã quy định."
            ...
            userPrompt: prompt 
              ? `Thể loại: ${genresStr}\nYêu cầu gốc: ${prompt}\n\nLỆNH ÉP BUỘC ĐA DẠNG (EXTREME DIVERSITY): Mặc dù chung 1 yêu cầu gốc, nhưng ${targetChapters} kịch bản PHẢI VÔ CÙNG KHÁC BIỆT. Cấm lặp lại nghề nghiệp (không phải lúc nào cũng giao hàng/bảo vệ), cấm lặp lại bối cảnh (phải đổi sang đám cưới, nhà tang lễ, du thuyền, v.v.), cách đặt tên cũng phải khác concept nhau hoàn toàn. Không được có 2 kịch bản na ná nhau!` 
              : `Thể loại: ${genresStr}\nYêu cầu: Dựa vào thể loại trên, hãy tạo ${targetChapters} kịch bản VÔ CÙNG KHÁC BIỆT NHAU về bối cảnh, nghề nghiệp nhân vật, twist và cách đặt tên. NGHIÊM CẤM tạo các kịch bản có motip na ná nhau. Mỗi kịch bản là một luồng gió hoàn toàn mới.`,
            jsonMode: true,
            temperature: 1.0,
...
NHIỆM VỤ CỦA BẠN: Viết trực tiếp (KHÔNG DÙNG JSON) 1 ĐOẠN ĐỀ XUẤT CÁCH SỬA CỤ THỂ CHO BÁC SĨ (Ví dụ: "Đối với Kịch bản 1: Sửa lại cú lật bàn cho ác độc hơn, cho nam phụ phản bội từ tập 1. Còn Kịch Bản 2: ..."). Lời khuyên phải sắc bén, đi thẳng vào trọng tâm để Bác sĩ có thể copy quăng vào lò phẫu thuật (Auto-Fix) ngay. Đừng viết quá dài, gạch đầu dòng rõ ràng.`,
  ...
  const gradeAllPitches = async () => {
     ...
     setIsGradingParam(-1);
     try {
         const res = await agentConceptScorer('qwen', qwenKey, selectedModel || 'qwen3-max-2026-01-23', pitchOptions, resolvedGKey2, resolvedGKey3);
