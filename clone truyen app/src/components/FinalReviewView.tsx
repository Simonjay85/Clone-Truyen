...
export function FinalReviewView() {
  ...
  }, [selectedItem?.id, selectedItem?.publishData]);
  ...
  const generateCoverPolli = (promptStr: string) => {
      // Remove specific prefixes like "A cover image of..." generated by LLMs to save tokens and improve polli quality
      return `https://image.pollinations.ai/prompt/${encodeURIComponent(promptStr + " --no-logo")}?width=800&height=1200&nologo=true&seed=${Math.floor(Math.random()*1000)}`;
  };
  ...
  const handlePublishToWeb = async (item: QueueItem) => {
    ...
    try {
      const pData = item.publishData || {
         ...
         coverUrl: '',
         seoTitle: item.title,
         seoDescription: item.bible?.summary || item.bible?.series_premise || item.prompt,
         seoFocusKeyword: '',
         blurb: item.bible?.summary || item.bible?.series_premise || item.prompt
      };
      
      const { finalTitle, categories, tags, coverUrl, seoTitle, seoDescription, seoFocusKeyword, blurb } = pData;
      ...
      let htmlIntro = coverUrl ? `<div style="text-align: center; margin-bottom: 20px;"><img src="${coverUrl}" alt="${finalTitle}" style="max-width: 100%; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.2);" /></div>` : '';
      ...
      const storyIntro = blurb || item.bible?.series_premise || item.bible?.summary || item.prompt;
      await callWordPress({
         wpUrl, wpUser, wpAppPassword,
         endpoint: `truyen/${item.wpPostId}`,
         method: 'POST',
                  ...
                  {activeTab === 'bible' && (
                     <div className="max-w-[1200px] mx-auto space-y-8 animation-fade-in text-[15px] text-slate-300 leading-[1.8] font-medium pb-10">
                        <div className="bg-[#17172a] border border-white/5 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                           ...
                           <div className="whitespace-pre-wrap">{selectedItem.bible?.series_premise || selectedItem.bible?.summary || selectedItem.prompt}</div>
                        </div>
                        
                        {selectedItem.bible?.character_bible ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="bg-[#17172a] border border-emerald-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                   ...
                                   <div className="whitespace-pre-wrap">{selectedItem.bible?.character_bible || 'Chưa có thông tin'}</div>
                                </div>
                                ...
                                <div className="space-y-8">
                                   <div className="bg-[#17172a] border border-amber-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                      ...
                                      <div className="whitespace-pre-wrap">{selectedItem.bible?.hidden_secrets_map || 'Chưa có thông tin'}</div>
                                   </div>
                                   <div className="bg-[#17172a] border border-rose-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                      ...
                                      <div className="whitespace-pre-wrap">{selectedItem.bible?.emotional_escalation_ladder || 'Chưa có thông tin'}</div>
                                   </div>
                                   <div className="bg-[#17172a] border border-fuchsia-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                      ...
                                      <div className="whitespace-pre-wrap">{selectedItem.bible?.forbidden_inconsistencies || 'Chưa có thông tin'}</div>
                                   </div>
                                </div>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="bg-[#17172a] border border-emerald-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                   ...
                                   <div className="whitespace-pre-wrap">{selectedItem.bible?.worldSettings || 'Chưa có thông tin'}</div>
                                </div>
                                <div className="bg-[#17172a] border border-amber-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                   <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-amber-500"></div>
                                   <h3 className="text-amber-400 font-black mb-4 uppercase tracking-widest text-sm">🤕 Arc Nhân Vật / Nỗi Đau</h3>
                                   <div className="whitespace-pre-wrap">{selectedItem.bible?.characterArc || 'Chưa có thông tin'}</div>
                                </div>
                                <div className="bg-[#17172a] border border-rose-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                   <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-rose-500"></div>
                                   <h3 className="text-rose-400 font-black mb-4 uppercase tracking-widest text-sm">🌪️ Cú Twist Lật Bàn</h3>
                                   <div className="whitespace-pre-wrap">{selectedItem.bible?.plotTwists || 'Chưa có thông tin'}</div>
                                </div>
                                <div className="bg-[#17172a] border border-fuchsia-500/20 p-8 rounded-2xl shadow-lg relative overflow-hidden">
                                   <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-fuchsia-500"></div>
                                   <h3 className="text-fuchsia-400 font-black mb-4 uppercase tracking-widest text-sm">🔥 Độ Bạo Não Truyện</h3>
                                   <div className="whitespace-pre-wrap">{selectedItem.bible?.overallSizzle || 'Chưa có thông tin'}</div>
                                </div>
                            </div>
                        )}
                     </div>
                                ...
                                <div className="flex-1 bg-[#17172a] border border-white/5 rounded-2xl overflow-y-auto custom-scrollbar p-6 md:p-10 text-[#cbd5e1] relative shadow-xl">
                                     {typeof readingChapterIdx === 'number' && selectedItem.chaptersContent[readingChapterIdx] && (
                                         <div className="max-w-3xl mx-auto pb-10">
                                             ...
                                             {selectedItem.bible?.timeline?.[selectedItem.chaptersContent[readingChapterIdx].episode - 1]?.outline && (
                                                 <div className="bg-amber-500/5 border-l-4 border-amber-500 p-5 mb-10 rounded-r-xl shadow-sm">
                                                     ...
                                                     <div className="text-[15px] font-semibold text-amber-200/90 italic leading-relaxed border-t border-amber-500/10 pt-2">{selectedItem.bible?.timeline[selectedItem.chaptersContent[readingChapterIdx].episode - 1]?.outline}</div>
                                                 </div>
                                             )}
