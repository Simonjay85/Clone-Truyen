...
export function ComboEconomicView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  const { geminiKey, geminiKey2, geminiKey3, geminiPaidKey, usePaidAPI, addQueueItems, draftSpaces, updateDraftSpace, addApiLog } = useStore();
  ...
  const draft = draftSpaces['combo_eco'] || {};
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      updateDraftSpace('combo_eco', { title, prompt, targetChapters, pitchOptions, gradingStatus });
    }, 500);
...
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY CÓ ĐÚNG 5 KỊCH BẢN.
{
  ...
  const gradePitch = async (index: number) => {
     ...
     try {
        if (!geminiKey) return alert("Chưa có OpenAI Key");
        const res = await fetch('/api/gemini', {
           method: 'POST',
           headers: { 'Content-Type': 'application/json' },
           body: JSON.stringify({
...
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY CÓ ĐÚNG ${targetChapters} KỊCH BẢN.
{
  ...
  const handlePushAutopilot = () => {
    ...
    const items = selectedPitches.map(idx => ({
        ...
        model: selectedModel,
        comboType: 5 as const, outlineEngine: 'gemini' as const, writeEngine: 'openai' as const, isAdvancedPipeline: true
    }));
    addQueueItems(items);
    setSelectedPitches([]);
    ...
    <div className="w-full h-full flex flex-col font-sans bg-[#0f0f17] text-[#e2e8f0] overflow-hidden animation-fade-in">
      ...
      <nav className="h-auto md:h-[60px] py-4 md:py-0 bg-[rgba(15,15,23,0.9)] border-b border-[rgba(255,255,255,0.07)] px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 shrink-0 z-50 backdrop-blur-md">
        <div className="flex items-center gap-2 font-extrabold text-[18px] text-white tracking-wide">
          🎬 Trạm Đạo Diễn <span className="bg-gradient-to-br from-emerald-500 to-emerald-400 text-white px-2.5 py-0.5 rounded-full text-xs uppercase tracking-wider ml-1">OpenAI</span>
        </div>
        
        <div className="flex gap-3">
           <button className="px-6 py-2 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-500 text-white font-bold text-sm shadow-lg">🎬 Đạo Diễn</button>
        </div>
              ...
              {!isGenerating && pitchOptions.length === 0 && (
                 <div className="h-full flex flex-col items-center justify-center text-center opacity-70">
                    ...
                    <p className="text-[15px] text-[#374151] max-w-[420px] leading-relaxed font-medium">
                       Nhập ý tưởng câu chuyện của bạn vào bảng điều khiển bên trái, sau đó bấm <strong>&quot;💡 Gợi ý Kịch Bản&quot;</strong>. Cỗ máy AI sẽ thiết lập các kịch bản cực gắt và bạn có thể Nhờ Giám Khảo Supreme Judge chấm điểm rủi ro!
                    </p>
                 </div>
              )}
                          ...
                          <tbody className="divide-y divide-white/5">
                             {pitchOptions.map((p, i) => ({ pitch: p, index: i, grade: gradingStatus[i] }))
                                ...
                                <tr key={item.index} className={`hover:bg-white/5 transition-colors ${selectedPitches.includes(item.index) ? 'bg-teal-500/10' : ''}`}>
                                   ...
                                   <td className="px-4 py-3 font-bold text-white max-w-[200px] truncate" title={((item.pitch as any) as any)?.super_title || ((item.pitch as any) as any)?.protagonist}>{((item.pitch as any) as any)?.super_title || ((item.pitch as any) as any)?.protagonist}</td>
