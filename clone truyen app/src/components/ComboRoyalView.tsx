  ...
  const gradePitch = async (index: number) => {
     ...
     try {
        ...
        const res = await fetch('/api/claude', {
           ...
           body: JSON.stringify({
              apiKey: claudeKey,
              systemPrompt: `Bạn là Đạo Diễn Thiết Lập xuất sắc của Anthropic. Hãy viết cốt truyện có chiều sâu tâm lý, tinh tế và đầy ám ảnh. TẠO ĐÚNG ${targetChapters} KỊCH BẢN MICRO-DRAMA.
TUYỆT ĐỐI KHÔNG DÙNG TỪ TIẾNG ANH. Viết bằng tiếng Việt hoặc Hán Việt thuần túy. Xây dựng cốt truyện mang màu sắc Châu Á (đặc biệt là Việt Nam hoặc Trung Quốc).
