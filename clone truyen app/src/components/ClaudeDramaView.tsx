...
export function ClaudeDramaView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const draft = draftSpaces['claude_drama'] || {};
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      updateDraftSpace('claude_drama', { title, prompt, targetChapters, pitchOptions, gradingStatus });
    }, 500);
...
BẮT BUỘC TRẢ VỀ JSON OBJECT CÓ KEY "pitches" LÀ ARRAY CÓ ĐÚNG 5 KỊCH BẢN.
{
  ...
  const gradePitch = async (index: number) => {
     ...
     try {
        ...
        const res = await agentConceptScorer('claude', claudeKey, 'claude-3-5-sonnet-20241022', [pitch]);
        if (res.scores && res.scores.length > 0) {
           const evalData = res.scores[0];
           setGradingStatus(prev => ({ ...prev, [index]: { score: evalData.score || 0, grading: evalData.reason || evalData.grading || '' } }));
        }
     } catch (e: any) {
  ...
  const handlePushAutopilot = () => {
    ...
    const items = selectedPitches.map(idx => ({
        ...
        model: selectedModel,
        outlineEngine: 'claude' as const, writeEngine: 'claude' as const
    }));
    addQueueItems(items);
    setSelectedPitches([]);
                ...
                {Object.entries(GENRES_GROUPS).map(([groupName, genres]) => (
                   <div key={groupName}>
                     ...
                     <div className="flex flex-wrap gap-2">
                        {genres.map(g => (
                           <button key={g} onClick={() => toggleGenre(g)} className={`px-3 py-1.5 rounded-full border text-xs font-semibold transition-all ${selectedGenres.includes(g) ? 'border-amber-500 text-amber-300 bg-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.15)]' : 'border-white/10 text-slate-400 hover:border-amber-500/50 hover:text-amber-300'}`}>{g}</button>
                        ))}
                     </div>
                   </div>
                ))}
           ...
           <div className="flex-1 md:overflow-y-auto custom-scrollbar p-10 relative text-[#e2e8f0]">
              ...
              {!isGenerating && pitchOptions.length === 0 && (
                 <div className="h-full flex flex-col items-center justify-center text-center opacity-70">
                    ...
                    <h2 className="text-[22px] font-bold text-[#4b5563] mb-4">Thiên Đường Sáng Tác (Claude Wordsmith)</h2>
