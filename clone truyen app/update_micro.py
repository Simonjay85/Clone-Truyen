...
with open('src/components/MicroDramaView.tsx', 'r') as f:
    content = f.read()

# 1. Add agentConceptScorer import
content = content.replace("import { agentPuppetMaster, callGemini } from '../lib/engine';", "import { agentPuppetMaster, callGemini } from '../lib/engine';\nimport { agentConceptScorer } from '../lib/advanced_engine';")

# 2. Add updatePitch function
content = content.replace("const togglePitchSelect = (idx: number) => {", "const updatePitch = (idx: number, field: string, value: string) => { const newP = listPitches(); newP[idx] = { ...newP[idx], [field]: value }; setPitchOptions(newP); }; const togglePitchSelect = (idx: number) => {")
content = content.replace("const newP = listPitches();", "const newPOptions = [...pitchOptions]; newPOptions[idx] = { ...newPOptions[idx], [field]: value }; setPitchOptions(newPOptions);")

# 3. Add gradeAllPitches function
grade_all_code = """
...
# 4. Replace individual grade button with nothing, since we use batch. Actually keep individual button but add batch button at the top header
batch_btn = """
...
content = re.sub(static_jsx_pattern, editable_jsx.replace('\\n', '\n'), content, flags=re.DOTALL)

with open('src/components/MicroDramaView.tsx', 'w') as f:
    f.write(content)
