...
.dtt-toc-toggle:hover,
.dtt-toc-toggle:focus-visible {
    box-shadow: 0 4px 10px rgba(91, 91, 214, .18);
    transform: translateY(-1px);
}
@media (max-width: 1199px) {
...
<?php if ($dtt_is_kiem_lai_wiki) : ?>
    <link rel="stylesheet" href="<?php echo esc_url(get_template_directory_uri() . '/assets/css/kiem-lai-wiki.css?ver=1.0.1'); ?>">
<?php else : ?>
    <link rel="stylesheet" href="<?php echo esc_url(get_template_directory_uri() . '/assets/css/single-post-sync.css?ver=1.0.1'); ?>">
<?php endif; ?>
    ...
    <div class="dtt-single-post__inner">
        <?php while (have_posts()) : the_post(); ?>
            ...
            if (preg_match('/<nav\b[^>]*aria-label=["\x27]Mục lục[^"\x27]*["\x27][^>]*>.*?<\/nav>/is', $content_html, $toc_match)) {
                ...
                // before the TOC list. Remove that malformed wrapper so the
                // browser does not create an empty paragraph inside the rail.
