...
<div class="mkm-taxonomy-page" style="margin-top:20px; margin-bottom:40px; padding:0 15px; max-width:1200px; margin-left:auto; margin-right:auto;">
    ...
    <div class="mkm-tax-filters" style="margin-bottom: 24px;">
        ...
        <div class="mkm-genre-chips" style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:20px;">
            <a href="<?php echo get_site_url(); ?>/?post_type=truyen" class="mkm-chip active">Tất cả</a>
            <?php foreach($all_terms as $t): 
                $link = get_term_link($t);
            ?>
                <a href="<?php echo esc_url($link); ?>" class="mkm-chip"><?php echo esc_html($t->name); ?></a>
            <?php endforeach; ?>
        </div>
    ...
    <section class="mkm-grid">
        ...
        <a href="<?php the_permalink(); ?>" class="mkm-card">
            <div class="mkm-card-img">
                ...
                <div class="mkm-badge-tl">MỚI</div>
...
.mkm-chip { padding: 8px 16px; border-radius: 99px; background: #f3f4f6; color: #4b5563; font-size: 14px; font-weight: 600; text-decoration: none; transition: all 0.2s; }
.mkm-chip:hover { background: #e5e7eb; }
.mkm-chip.active { background: #4f46e5; color: #fff; box-shadow: 0 4px 6px -1px rgba(79,70,229,0.3); }
.mkm-grid { display: grid !important; grid-template-columns: repeat(4, 1fr) !important; gap: 14px !important; margin-bottom: 0 !important; }
@media (max-width: 900px) { .mkm-grid { grid-template-columns: repeat(3, 1fr) !important; } }
@media (max-width: 600px) { .mkm-grid { grid-template-columns: repeat(2, 1fr) !important; } }
.mkm-card { background: #fff !important; border-radius: 12px !important; overflow: hidden !important; box-shadow: 0 1px 4px rgba(0,0,0,0.06) !important; transition: box-shadow .2s, transform .2s !important; text-decoration: none !important; display: block !important; }
.mkm-card:hover { box-shadow: 0 6px 20px rgba(0,0,0,0.12) !important; transform: translateY(-2px) !important; }
.mkm-card-img { position: relative !important; aspect-ratio: 3/4 !important; overflow: hidden !important; }
.mkm-card-img img { width: 100% !important; height: 100% !important; object-fit: cover !important; display: block !important; transition: transform .3s !important; }
.mkm-card:hover .mkm-card-img img { transform: scale(1.05) !important; }
.mkm-badge-tl { position: absolute !important; top: 8px !important; left: 8px !important; background: #10b981 !important; color: #fff !important; font-size: 10px !important; font-weight: 700 !important; padding: 2px 7px !important; border-radius: 6px !important; z-index: 2 !important; }
.mkm-card-overlay { position: absolute !important; bottom: 0 !important; left: 0 !important; right: 0 !important; background: linear-gradient(0deg, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.4) 60%, transparent 100%) !important; padding: 20px 10px 10px 10px !important; display: flex !important; flex-direction: column !important; justify-content: flex-end !important; z-index: 2 !important; }
.mkm-card-title { color: #fff !important; font-size: 14px !important; font-weight: 700 !important; line-height: 1.3 !important; margin-bottom: 6px !important; display: -webkit-box !important; -webkit-line-clamp: 2 !important; -webkit-box-orient: vertical !important; overflow: hidden !important; }
.mkm-card-meta { display: flex !important; justify-content: space-between !important; align-items: center !important; color: #f3f4f6 !important; font-size: 11px !important; font-weight: 600 !important; }
.mkm-pagination a, .mkm-pagination span { display: inline-flex; align-items: center; justify-content: center; padding: 8px 16px; margin: 0 4px; border-radius: 8px; background: #f3f4f6; color: #374151; font-weight: 600; text-decoration: none; font-size: 14px; transition: all 0.2s; }
.mkm-pagination a:hover { background: #e5e7eb; }
.mkm-pagination .current { background: #4f46e5; color: #fff; box-shadow: 0 4px 6px -1px rgba(79,70,229,0.3); }
</style>
