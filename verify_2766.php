...
$total = count($chs);
$last = $total > 0 ? end($chs) : null;
echo json_encode([
    'total' => $total,
    'chapters' => array_map(fn($c) => ['id'=>$c->ID,'title'=>$c->post_title,'len'=>strlen($c->post_content)], $chs),
    'last_content' => $last ? substr(strip_tags($last->post_content), -800) : null,
