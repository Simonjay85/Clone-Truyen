...
# -*- coding: utf-8 -*-
"""
publish_batch_v2.py — Batch Markdown Parser and WordPress Publisher for V2 Stories
==================================================================================
Parses the user's batch of 9 rewritten sảng văn novels from:
/Users/aaronnguyen/Downloads/rewrite-9-truyen-v2.md
...
COVER_TEMPLATES = {
    1: {"base": "base_cover_20.png", "author": "Lê Minh Khoa & Trần Bảo Châu", "sub": "Bản hùng ca sảng văn của Lê Minh Khoa & Trần Bảo Châu"},
    2: {"base": "base_cover_21.png", "author": "Ngô Thanh Vy & Trần Minh Triết", "sub": "Bản hùng ca sảng văn của Ngô Thanh Vy & Trần Minh Triết"},
    3: {"base": "base_cover_22.png", "author": "Tô Vân Khanh & Mộ Dương Thành", "sub": "Bản hùng ca sảng văn của Tô Vân Khanh & Mộ Dương Thành"},
    4: {"base": "base_cover_23.png", "author": "Lý Ngọc Uyển & Vương Tử Diễm", "sub": "Bản hùng ca sảng văn của Lý Ngọc Uyển & Vương Tử Diễm"},
    5: {"base": "base_cover_24.png", "author": "Dương Hải Bình & Lê Thu Vân", "sub": "Bản hùng ca sảng văn của Dương Hải Bình & Lê Thu Vân"},
    6: {"base": "base_cover_25.png", "author": "Nguyễn Phương Anh & Lý Khải Minh", "sub": "Bản hùng ca sảng văn của Nguyễn Phương Anh & Lý Khải Minh"},
    7: {"base": "base_cover_26.png", "author": "Trần Minh Châu & Vũ Hoàng Nam", "sub": "Bản hùng ca sảng văn của Trần Minh Châu & Vũ Hoàng Nam"},
    8: {"base": "base_cover_27.png", "author": "Nguyễn Trọng Khang & Trần Thảo Vy", "sub": "Bản hùng ca sảng văn của Nguyễn Trọng Khang & Trần Thảo Vy"},
    9: {"base": "base_cover_28.png", "author": "Lý Sương Nguyệt & Trần Hạo Thiên", "sub": "Bản hùng ca sảng văn của Lý Sương Nguyệt & Trần Hạo Thiên"},
}
...
def parse_markdown_file(file_path):
    ...
    # Split content by stories using `# TRUYỆN [1-9]: ` headings
    story_blocks = re.split(r'(?m)^# TRUYỆN ([1-9]):\s+', content)
    
    # The first element is the file header (e.g., "# BATCH 9 TRUYỆN...")
    ...
    for i in range(0, len(blocks), 2):
        ...
        # Parse Introduction
        # Introduction is between ## Giới thiệu and ## Chương 1
        intro_match = re.search(r'## Giới thiệu\s*\n(.*?)\n\s*##\s*Chương 1', remaining_text, re.DOTALL)
        if not intro_match:
            # Fallback
            intro_match = re.search(r'## Giới thiệu\s*\n(.*?)\n\s*##\s*Chương', remaining_text, re.DOTALL)
...
def execute_dry_run(stories):
    print("\n" + "=" * 80)
    print("🔬 DRY-RUN PARSE VERIFICATION REPORT (BATCH V2)")
    print("=" * 80)
...
def execute_live_publishing(stories):
    log("🚀 STARTING LIVE PUBLICATION PIPELINE FOR BATCH V2")
    ...
    for s in stories:
        ...
        log(f"\n============================================================")
        log(f"📖 PUBLISHING STORY {idx}/9: {title}")
        log(f"============================================================")
    ...
    log(f"  -> Total successfully published: {published_count}/9")
    if failed_indices:
...
# ─── MAIN ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Parse and publish batch v2 sảng văn novels")
    parser.add_argument("--file", default="/Users/aaronnguyen/Downloads/rewrite-9-truyen-v2.md", help="Path to markdown novel batch file")
